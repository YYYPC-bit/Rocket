...
def sudo(client, message,redis):
	...
	if text and (type == enums.ChatType.PRIVATE or (type == enums.ChatType.SUPERGROUP or type == enums.ChatType.GROUP)) :
		...
		if rank == "sudo" or rank == "asudo":
			...
			if text == c.ADDfiles:
				url = "https://raw.githubusercontent.com/YYYPC-bit/Rocket-files/master/files"
			...
			if text == c.Ubot:
				...
				for fnh in Files_H:
					url = "https://raw.githubusercontent.com/YYYPC-bit/Rocket/master/handlers/"+fnh
					out = requests.get(url).text
					f = open("./handlers/"+fnh,"w+")
					f.write(out)
					f.close()
				for fnu in Files_U:
					url = "https://raw.githubusercontent.com/YYYPC-bit/Rocket/master/utlis/"+fnu
					out = requests.get(url).text
					f = open("./utlis/"+fnu,"w+")
					f.write(out)
					f.close()
				for fnb in Files_B:
					url = "https://raw.githubusercontent.com/YYYPC-bit/Rocket/master/"+fnb
					out = requests.get(url).text
					f = open("./"+fnb,"w+")
					f.write(out)
					f.close()
				for fnu in Files_L:
					url = "https://raw.githubusercontent.com/YYYPC-bit/Rocket/master/lang/"+fnu
					out = requests.get(url).text
					f = open("./lang/"+fnu,"w+")
					f.write(out)
					f.close()
